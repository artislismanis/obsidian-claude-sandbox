...
describe("Plugin loads", function () {
	...
	it("adds the container status bar item", async function () {
		// At least one status-bar item exists (plugin adds two: container + firewall)
		await expect($(".status-bar .status-bar-item")).toBeDisplayed();
	});
...
describe("Settings UI", function () {
	...
	it("auto-generates an MCP token on first load", async function () {
		const token = await browser.executeObsidian(({ plugins }) => {
			const p = plugins["obsidian-agent-sandbox"];
			return p.settings.mcpToken;
		});
...
describe("Container status (no Docker required)", function () {
	it("starts in the stopped state when plugin loads", async function () {
		// With no Docker compose path configured, probeStatus will fail
		// and the status bar should show 'error' state. Either stopped
		// or error is acceptable — we just want to verify the plugin
		// didn't crash trying to talk to Docker.
		const statusBar = $(".status-bar .status-bar-item");
